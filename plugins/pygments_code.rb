...
module HighlightCode
  ...
  def highlight(code, options = {})
    ...
    unless cache
      ...
      code = pre_code(code, lang, {linenos: linenos, start: start, marks: marks })
      title = captionize(title, url, link_text) if title
      code = title ? "<figure class='code caption'>#{title}#{code}</figure>" : "<figure class='code'>#{code}</figure>"
  ...
  def pre_code (code, lang, options = {})
    start = options[:start] || 1
    lines = options[:linenos] || true
    marks = options[:marks] || []
    pre = "<div class='highlight'>"
    pre += "<pre class='main #{'unnumbered' unless lines} #{lang}'>"
    code.lines.each_with_index do |line,index|
      ...
      pre += "<div class='#{classes}'>#{line}</div>"
    end
    pre +="</pre></div>"
  end

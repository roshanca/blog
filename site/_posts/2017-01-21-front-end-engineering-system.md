...
至于如何做到自动化以提升效率，主要利用 git hooks 通知到 CI 服务器，执行对于脚本来做，这里有许多方案可供选择。由于我在公司一直用的都是 gitlab，所以之前只尝试过 gitlab-ci，能与 Docker 配合着使用，用起来还是比较爽的。

其它比较常见的就是譬如 Jenkins, Travis CI 等。
